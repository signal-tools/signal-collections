...
if (signalArgument === undefined || collectionsArgument === undefined) {
	throw new Error("Expected Signal and collections tarball paths");
}

const signalTarball = resolve(signalArgument);
const collectionsTarball = resolve(collectionsArgument);
const consumer = mkdtempSync(join(tmpdir(), "signal-tools-consumer-"));

try {
	writeFileSync(join(consumer, "package.json"), '{"private":true,"type":"module"}\n');
	execFileSync("npm", ["install", "--no-audit", "--no-fund", signalTarball, collectionsTarball], {
		cwd: consumer,
		stdio: "inherit",
	});
	...
	console.log(`Validated ${basename(signalTarball)} with ${basename(collectionsTarball)}`);
} finally {
